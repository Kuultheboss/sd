...
local DAMAGE_AMOUNT = 10  -- 10 HP kaupa
local DAMAGE_INTERVAL = 0.5  -- iga 0.5 sekundi tagant

local function findSpecificNPCs(center, radius)
    local foundNPCs = {}
    
    for _, npc in pairs(workspace:GetChildren()) do
        for _, targetName in pairs(targetNPCNames) do
            if npc.Name == targetName then
                if npc:FindFirstChild("Humanoid") and npc:FindFirstChild("HumanoidRootPart") then
                    local distance = (npc.HumanoidRootPart.Position - center).Magnitude
...
local function NPCManagement()
    while true do
        ...
        local nearbyNPCs = findSpecificNPCs(center, radius)
        
        for _, npc in pairs(nearbyNPCs) do
            local humanoid = npc:FindFirstChild("Humanoid")
            if humanoid and humanoid.Health > 0 then
                -- Anna 10 HP kahju, kuidä jäta vähemalt 1 HP alles
                local newHealth = math.max(1, humanoid.Health - DAMAGE_AMOUNT)
                humanoid.Health = newHealth
                
                -- Või kasuta TakeDamage meetodit
